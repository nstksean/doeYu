    ...
    return (
        <div className={style.stackGruop}>
            ...
            <div className={style.stackSlide}>
                <div
                    className={style.stackPre}
                    onClick={() =>
                        setActiveTab(activeTab < 1 ? len : activeTab - 1)
                    }
                >
                    <svg
                        fill="none"
                ...
                <div className={style.stackContainer}>
                    {citys
                        .filter((city) => city.category === filter.name)
                        .map((city) => (
                            <CityCard city={city} key={city?.key} />
                        ))}
                </div>
                <div
                    className={style.stackNex}
                    onClick={() =>
                        setActiveTab(activeTab > 2 ? 0 : activeTab + 1)
                    }
                >
                    <svg
                        fill="none"
